 ...
 */
class ChartShapeMarker extends ChartMarker {
...
//@}
     ...
     *
     * To create a border to a shape with  Chart API, you need to create
    ...
    public function compute($index, $chart_type = null) {
        ...
        if ($this->border !== null) {
            $str .= sprintf(
                    '%s,%s,%d,%s,%s|', $this->shape, $this->border['color'], $index, $points, $this->size + $this->border['size']
            );
        }

        $str .= sprintf(
                '%s,%s,%d,%s,%s', $this->shape, $this->color, $index, $points, $this->size
        );
