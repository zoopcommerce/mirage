...
class ChartData {
     ...
     * Default color by  Chart API is ffcc33
     */
     ...
     * @since 0.5
     */
    public function computeChd($encoding = Chart::TEXT, $scale = null) {
...
//@}
     ...
     *
     * @internal
     * @note Used by Chart when calling Chart::addData()
     * @param $index (int)
     * @return $this
     */
...
//@{
    ...
    public function computeChm($index) {
        ...
        if ($this->fill !== null) {
            $fill[] = sprintf(
                    '%s,%s,%d,%d,0', $this->fill['end_line'] === null ? 'B' : 'b', $this->fill['color'], $index, $this->fill['end_line']
            );
        }
        ...
        if (isset($this->fill_slices[0])) {
            foreach ($this->fill_slices as $f) {
                $fill[] = sprintf(
                        'B,%s,%d,%s:%s,0', $f['color'], $index, $f['start'], $f['stop']
                );
            }
        }
        return implode('|', $fill);
...
//@}
    ...
    /**
     * Function that creates a new ChartData element with trend points based
     * on the current values.
     *
     * @return ChartData Trend data
     */
     ...
     * data.
     *
     * @return ChartLineMarker Trend line
     */
